...
- `20260107T1432Z` — **Remove unused demo and assets**: Deleted unused demo components (`components/component-example.tsx`, `components/example.tsx`), unused UI primitives only referenced by the demo (`components/ui/alert-dialog.tsx`, `components/ui/badge.tsx`, `components/ui/combobox.tsx`, `components/ui/dropdown-menu.tsx`, `components/ui/input-group.tsx`, `components/ui/select.tsx`, `components/ui/textarea.tsx`), unused SVGs in `public/`, removed a no-op layout (`app/(portal)/layout.tsx`), and pruned unused exports (`actions/auth.ts`, `lib/crypto.ts`, `lib/url-parser.ts`).
- `20260107T1436Z` — **Make builds offline-safe**: Removed `next/font/google` usage (which fetches fonts during build) and set CSS font variables to system stacks in `app/globals.css`.
